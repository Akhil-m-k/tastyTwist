  ...
  return (
    <div key={identifier}>
      {isVisible ? (
        <div
          key={identifier + 1}
          className="cursor-pointer text-2xl flex justify-between font-medium bg-gray-200 p-3 mb-2"
        ...
        <div
          key={identifier + 2}
          className="cursor-pointer flex justify-between text-2xl font-medium bg-gray-200 p-3 mb-2"
      ...
      {isVisible && (
        <div key={identifier + 3}>
          {restauro.itemCards.map((item, index) => (
            <div className="item-card flex flex-col sm:flex-row border-b-2 py-4" key={index + identifier}>
              <div className="pr-3 max-w-28 flex flex-col items-center justify-center  mt-2 sm:mt-0">
                <img
                  ...
                  alt=""
                  className="rounded-md min-h-20 w-full"
                />
                <button
                  className="bg-green-600 px-4 rounded-md text-white shadow w-full p-1 mt-2"
              ...
              <div className="">
                <h3 className="text-2xl font-bold">{item.card.info.name}</h3>
                <h5 className="text-gray-500">{item.card.info.description}</h5>
      ...
      <div className="max-w-screen-xl py-5 mx-auto sm:px-10 mt-4 mb-8" key={2}>
        {restauroMenu.map((restauro, i) => {
          if (restauro.card.card.itemCards) {
            ...
            return (
              <React.Fragment key={restauro.card.card.title}>
